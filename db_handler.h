...
class Db_handler : public QSqlDatabase {
 public:
  Db_handler(QString driver, QString host, QString dbName, QString user,
             QString passw);
  ~Db_handler();
  bool isOk();
  ...
  bool delete_item(const QString &in) const;
  bool delete_job(const QString &in) const;
  ...
  bool bind_job_to_item(QString &item, QString &job);
