...
DataTable::DataTable(QWidget *parent, MainWindow *window)
  ...
  d_reset->setText("Сбросить");
  d_layout->addWidget(d_title);
  d_layout->addWidget(d_table);
  d_layout->addWidget(d_type);
  d_layout->addWidget(d_reset);

  connect(d_type, SIGNAL(currentIndexChanged(int)), this,
          SLOT(show_table(int)));
  connect(d_table, SIGNAL(cellClicked(int, int)), this,
          SLOT(set_passport_data(int, int)));
...
ItemJobsTable::ItemJobsTable(QWidget *parent, MainWindow *window,
                             QString item_id)
    : DataTable(parent, window), d_item_id(item_id) {
  d_title->setText("Процессы для текущего изделия");

  d_append = new QPushButton();
  d_append->setText("Добавить");
  d_layout->addWidget(d_append);
  connect(d_append, SIGNAL(clicked(bool)), this, SLOT(on_d_append_clicked()));

  d_remove = new QPushButton();
  d_remove->setText("Удалить");
  d_layout->addWidget(d_remove);
  connect(this->d_remove, SIGNAL(clicked(bool)), this,
          SLOT(on_d_remove_clicked()));
  ...
  d_add_ok->setText("Добавить в базу данных");
  d_add_layout->addWidget((d_add_ok));
  connect(this->d_add_ok, SIGNAL(clicked(bool)), this,
          SLOT(on_d_add_ok_clicked()));
  // end of table
}
...
void ItemJobsTable::show_table(int a) {
  d_window->handler->fillItJRelationTable(d_window->get_it_j(), d_table,
                                          d_item_id);
}

void ItemJobsTable::on_d_append_clicked() { d_add_widget->show(); }

void ItemJobsTable::on_d_add_ok_clicked() {
        QString cur_id = d_window->passport->get_id();
        QString cur_job = d_all_jobs_table->item(d_all_jobs_table->currentRow(), 0)->text();
        d_window->handler->bind_job_to_item(cur_id, cur_job);
}
...
FilesTable::FilesTable(QWidget *parent, MainWindow *window, QString job_id)
  ...
  d_title->setText(QString("Файлы процесса %1").arg(job_id));

  f_append = new QPushButton();
  f_append->setText("Добавить");
  d_layout->addWidget(f_append);
  connect(f_append, SIGNAL(clicked(bool)), this, SLOT(on_f_append_clicked()));

  f_remove = new QPushButton();
  f_remove->setText("Удалить");
  d_layout->addWidget(f_remove);
  connect(this->f_remove, SIGNAL(clicked(bool)), this,
          SLOT(on_f_remove_clicked()));
}
...
void FilesTable::on_f_append_clicked() {
  ...
  *f_filenames = dialog->getOpenFileNames();
  ...
  if (!(f_filenames->isEmpty())) {
    d_window->handler->set_files_info(f_filenames,
                                      d_window->passport->get_id());
  }
}
