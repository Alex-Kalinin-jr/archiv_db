...
Db_handler::Db_handler(QString driver, QString host, QString dbName,
                       QString user, QString passw)
...
bool Db_handler::isOk() { return okOpen_h; }

// fills tables with relevant data from db
bool Db_handler::set_data(db_struct *table) {
  if (okOpen_h) {
    table->t_data->clear();
    QSqlQuery my(db);
    QString a = QString("select %1 from %2;")
                    .arg(QString(table->t_fields))
...
// refills table with actual data
...
bool Db_handler::delete_item(const QString &in) const {
  if (okOpen_h) {
    QSqlQuery my(db);
...
bool Db_handler::delete_job(const QString &in) const {
  if (okOpen_h) {
    QSqlQuery my(db);
...
bool Db_handler::bind_job_to_item(QString &item, QString &job) {
  if (okOpen_h) {
    QSqlQuery my(db);
    QString a =
        QString("select * from item_job where items='%1' and jobs='%2';")
            .arg(item)
            .arg(job);
    my.exec(a);
    if (my.next()) {
...
bool Db_handler::fillJobsTable(db_struct *data, QTableWidget *table) {
  ...
  table->setHorizontalHeaderLabels({"Обозначение", "Наименование", "Тип"});
...
bool Db_handler::fillItemsTable(db_struct *data, QTableWidget *table) {
  ...
  QStringList *list = data->t_data;
  table->setColumnCount(3);
  table->setHorizontalHeaderLabels({"Обозначение", "Наименование", "Описание"});
  table->setRowCount(size);
  for (int i = 0; i < size; ++i) {
    for (int j = 0; j < 3; ++j) {
      cell = new QTableWidgetItem;
...
bool Db_handler::fillItJRelationTable(db_struct *data, QTableWidget *table,
                                      QString str) {
  table->clear();
  int size = data->t_data->size() / data->t_count;
  QTableWidgetItem *cell = nullptr;
  ...
  table->setHorizontalHeaderLabels({"Изделие №", "Процесс №"});
  table->setRowCount(0);
  for (int i = 0; i < size; ++i) {
...
bool Db_handler::fillFilesTable(db_struct *data, QTableWidget *table,
                                QString str) {
  table->clear();
  int size = data->t_data->size() / data->t_count;
  QTableWidgetItem *cell = nullptr;
  QStringList *list = data->t_data;
  table->setColumnCount(data->t_count);
  table->setHorizontalHeaderLabels({"Процесс №", "Файл (путь)"});
  table->setRowCount(0);
  for (int i = 0; i < size; ++i) {
    if (list->at((data->t_count) * i) == str) {
      table->setRowCount(table->rowCount() + 1);
